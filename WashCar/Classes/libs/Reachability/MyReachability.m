...
#pragma unused (target, flags)
	...
	NSCAssert([(NSObject*) info isKindOfClass: [MyReachability class]], @"info was wrong class in ReachabilityCallback");
    
	//We're on the main RunLoop, so an NSAutoreleasePool is not necessary, but is added defensively
	// in case someon uses the Reachablity object in a different thread.
	NSAutoreleasePool* myPool = [[NSAutoreleasePool alloc] init];
	
	MyReachability* noteObject = (MyReachability*) info;
...
+ (MyReachability*) reachabilityForLocalWiFi;
{
	...
	MyReachability* retVal = [self reachabilityWithAddress: &localWifiAddress];
	if(retVal!= NULL)
	{
