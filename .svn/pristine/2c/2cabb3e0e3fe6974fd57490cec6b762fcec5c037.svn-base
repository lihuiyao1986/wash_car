...
typedef NS_ENUM(int, PayloadMsgType)
{
    //作为状态通知，tilte=订单号，content=状态，多个参数的用逗号分开，目前适用于一个参数
    //作为派单消息，title=订单号，content=派单时间
    //作为用户消息，title是标题，content是内容
    PayloadMsgType_NormalMsg = 1 , // 1普通消息（含标题和内容）
    PayloadMsgType_PicMsg = 2 , // 2图片消息
    PayloadMsgType_LinkMsg = 3 , // 3链接消息
    PayloadMsgType_StatusChangeMsg = 101 ,  // 101状态通知
    PayloadMsgType_SendordersMsg = 102 // 102派单消息
};
...
-(void)httpErrorResp:(int)errorcode errorMsg:(NSString*)errorMsg
            dataType:(HttpRequestDataType)dataType
                 tag:(int)reqTag
{
    APP_DebugLog(@"errorMsg - %@ , reqTag - %d",errorMsg,reqTag);
}
...
-(void)httpSuccessResp:(NSDictionary*)result dataType:(HttpRequestDataType)dataType
                     tag:(int)reqTag
{
    APP_DebugLog(@"successMsg - %@,reqTag - %d",result,reqTag);
...
- (void)GexinSdkDidRegisterClient:(NSString *)clientId
{
    ...
    [CommonUtils saveGexinClientId:self.gxClientId];
    if (![CommonUtils loginUser])
    {
        return;
    }
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        NSDictionary *params = [NSDictionary dictionaryWithObjectsAndKeys:clientId,@"clientId",nil];
...
- (void)GexinSdkDidReceivePayload:(NSString *)payloadId fromApplication:(NSString *)appId
{
    ...
    if (payload)
    {
        ...
        dispatch_async(queue, ^{
            ...
            // 对推送消息处理
            [self analyzePayloadMsg:payloadMsg];
        });
    }
...
- (void) analyzePayloadMsg:(NSString*)payloadMsg
{
    // 1.截取个推消息
    ...
    switch (type) {
        case PayloadMsgType_StatusChangeMsg:
        {
            // 101状态通知
        ...
        case PayloadMsgType_SendordersMsg:
        {
            // 102派单消息
            // title=订单号，content=派单时间
            break;
        }
        case PayloadMsgType_NormalMsg:
        {
            // 1普通消息（含标题和内容）
            // title是标题，content是内容
            break;
        }
        case PayloadMsgType_PicMsg:
        {
            // 2图片消息
            break;
        }
        case PayloadMsgType_LinkMsg:
        {
            // 3链接消息
            break;
        }
