...
-(void)respResult:(NSDictionary*)result errorcode:(int)errorcode
    ...
    if (errorcode != HttpRespCode_RespSuceess)
    {
        ...
        {
            //需要登录
            __weak BaseController *this = self;
...
-(void)handleErrorCode:(int)errorCode errormsg:(NSString *)message tag:(int)reqTag
{
    ...
    // 处理服务端返回的错误消息
    NSString *errorMsg = [self handleErrorMsg:message tag:reqTag errorCode:errorCode];
    
    // 判断是否需要弹框
    if (alertError)
    {
        //如果错误信息为空，直接给一个默认的
        if([CommonUtils isStrEmpty:errorMsg])
        {
            errorMsg = DEFAULT_REQUEST_ERROR_MESSAGE;
        }
        //提示错误信息
        if(showErrWithToast)
        {
            [self showAlert:nil message:errorMsg type:AlertViewType_Toast];
        }
        else
        {
            [self showAlert:nil message:errorMsg type:AlertViewType_Alert];
        }
    }
    //处理请求错误的回调
    [self reqErrorHandle:errorCode errormsg:errorMsg tag:reqTag];
}

#pragma mark 处理服务端返回的消息
...
- (BOOL)locationServicesIsOpen
{
    ...
    {
        [self showAlert:@"提醒" message:@"定位服务已关闭,请进入系统设置开启" type:AlertViewType_Alert];
        return NO;
    }
    else
    {
