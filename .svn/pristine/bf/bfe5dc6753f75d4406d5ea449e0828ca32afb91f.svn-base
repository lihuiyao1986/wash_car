...
//协议左边的文字
#define protocalLeftLabelStr @"注册表示同意乐和车上的"
//验证码输入框提示信息
#define authcodeufPlaceholder @"请输入验证码"
//手机号输入框提示信息
#define mobileufPlaceholder @"请输入手机号"
//验证码已发送的提示
#define authcodeSendedTip @"验证码已发送"
//短信验证码没输的提示
#define authcodeIsNullTip @"亲，短信验证码不能为空哦!"
//手机号不能为空
#define mobileIsNullTip @"亲，手机号不能为空哦!"
//手机号码格式不正确的提示
#define mobileBadFormatTip @"请输入正确的手机号"
//获取短信验证码按钮没点击过的提示
#define authcodeBtnNotPressTip @"亲，请先获取短信验证码哦!"
//短信验证码格式不正确的提示
#define authcodeBadFormatTip @"请输入正确的短信验证码"
//页面标题
#define loginPageTitle @"输入手机号"
//手机号对应的高度
...
-(void)confirmBtnClicked:(UIButton*)btn
{
    ...
    if (!_hasGetAuthcode)
    {
        [self showAlert:authcodeBtnNotPressTip type:AlertViewType_Toast];
        return;
    }
...
{
    _hasGetAuthcode = YES;
    [self showAlert:authcodeSendedTip type:AlertViewType_Toast];
    [self beginTimer];
}
...
-(void)updateAuthcodeBtn:(NSTimer*)timer
{
    if (_currentSecond <= 0) {
        ...
        [_authcodeBtn setTitle:authcodeGetTitle forState:UIControlStateNormal];
        [_authcodeBtn setTitle:authcodeGetTitle forState:UIControlStateDisabled];
        [_authcodeBtn setTitle:authcodeGetTitle forState:UIControlStateHighlighted];
    }
    else
    {
        ...
        [_authcodeBtn setTitle:timeTipInfo forState:UIControlStateDisabled];
        [_authcodeBtn setTitle:timeTipInfo forState:UIControlStateHighlighted];
    }
    _currentSecond--;
}
...
-(BOOL)validateMobile
{
    ...
    if ([CommonUtils isStrEmpty:mobile])
    {
        [_viewShake shakeWithDuration:0.5f andView:_mobileuf completion:^{
            [self showAlert:mobileIsNullTip type:AlertViewType_Toast];
        }];
        return NO;
    }
    ...
    {
        [_viewShake shakeWithDuration:0.5f andView:_mobileuf completion:^{
            [self showAlert:mobileBadFormatTip type:AlertViewType_Toast];
        }];
        return NO;
    }
    return YES;
...
-(BOOL)validateAuthcode
{
    ...
    if ([CommonUtils isStrEmpty:authcode])
    {
        ...
        [_viewShake shakeWithDuration:0.5f andView:_authcodeuf completion:^{
            [this showAlert:authcodeIsNullTip type:AlertViewType_Toast];
        }];
        return NO;
    }
    else if(![[authcode trimNull]isSmsAuthcode])
    {
        __weak LoginController *this = self;
        [_viewShake shakeWithDuration:0.5f andView:_authcodeuf completion:^{
            [this showAlert:authcodeBadFormatTip type:AlertViewType_Toast];
        }];
        return NO;
    }
    return YES;
...
-(NavBarInfoModel*)navBarInfo
{
    ...
    {
        self.navCenterTitle = loginPageTitle;
    }
